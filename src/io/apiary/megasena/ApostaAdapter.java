...
public class ApostaAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView != null) {
			...
			viewHolder.tvNumConcurso.setText(fillWithZeros(aposta.getConcurso()
					.toString()));
			...
			for (Integer dezena : setDezenas) {
				String textoDezena = dezena.toString();
				if (dezena < 10) {
					StringBuilder stringBuilder = new StringBuilder();
					stringBuilder.append("0");
					stringBuilder.append(dezena.toString());
					textoDezena = stringBuilder.toString();
				}
				viewHolder.tvArray[i++].setText(textoDezena);
			}
	...
	private String fillWithZeros(String s) {
		...
		return stringBuilder.substring(stringBuilder.length() - STRING_SIZE);
	}
	...
	static class ViewHolder {
		...
		private int[] ids = new int[] { R.id.label_Concurso,
				R.id.label_Dezenas, R.id.textView3, R.id.textView4,
